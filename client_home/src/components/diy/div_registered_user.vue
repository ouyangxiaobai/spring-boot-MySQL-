<template>
	<div class="diy_details_box diy_div_registered_user">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "registered_user_id",
					source_id: this.obj["registered_user_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/registered_user/set?registered_user_id=' + _this.obj["registered_user_id"], {
								praise_len
							}, (res) => {
								if(res.result){

<template>
	<div class="diy_details_box diy_div_instrument_reservation">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "instrument_reservation_id",
					source_id: this.obj["instrument_reservation_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/instrument_reservation/set?instrument_reservation_id=' + _this.obj["instrument_reservation_id"], {
								praise_len
							}, (res) => {
								if(res.result){

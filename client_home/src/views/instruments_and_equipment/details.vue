<template>
	<div class="diy_details page_instruments_and_equipment" id="instruments_and_equipment_details">
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_instruments_and_equipment'>
							<div_instruments_and_equipment :obj="obj"></div_instruments_and_equipment>
						</div>
					</div>
				</div>
			</div>
...
<script>
	import div_instruments_and_equipment from "@/components/diy/div_instruments_and_equipment.vue";
	...
	export default {
		...
		data(){
			return{
				...
				form_comment: {
					...
					source_field: "instruments_and_equipment_id",
					source_id: 0,
					reply_to_id: 0,
				},
				count_pages: 1, // 页面数
		...
		methods:{
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/instruments_and_equipment/set?instruments_and_equipment_id=' + obj["instruments_and_equipment_id"], {
					hits
				}, (res) => {
					if(res.result){
						...
						var body = {
							source_table: "instruments_and_equipment",
							source_field: "instruments_and_equipment_id",
							source_id: this.obj.instruments_and_equipment_id,
			...
			get_obj_after(json) {
				...
				if (json && json.result && json.result.obj) {
					...
					this.form_comment.source_id = this.obj.instruments_and_equipment_id;
				}
			},
			/**
			 * 初始化收藏状态
			...
			get_collect(obj) {
				...
				var query = {
					source_table: "instruments_and_equipment",
					source_field: "instruments_and_equipment_id",
					source_id: obj["instruments_and_equipment_id"],
					user_id
				};
		 ...
		 /****
			...
			change_collect() {
				...
				if (this.state_collect) {
					...
					query.title = this.obj.instrument_name
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/instruments_and_equipment/set?instruments_and_equipment_id=' + _this.obj["instruments_and_equipment_id"], {
								praise_len
							}, (res) => {
								if(res.result){

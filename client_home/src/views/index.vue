		        ...
		        <div class="row">
		            <div class="col-12">
		                ...
		                <div class="diy_recommend">
		                    ...
		                    <div class="more_box">
		                        <router-link to="/instruments_and_equipment/list" class="more">
		                            <span>更多</span>
		                        </router-link>
		                    </div>
		                    <div class="switch" @click="show_list_instruments_and_equipment = !show_list_instruments_and_equipment">
		                        <div class="switch_box">
		                            <span v-if="show_list_instruments_and_equipment"> 切换表格 </span>
		                            <span v-else> 切换列表 </span>
		                        </div>
		                    </div>
		                </div>
		                <!--仪器设备推荐列表组件 -->
		                <list_instruments_and_equipment v-if="show_list_instruments_and_equipment" :list="list_instruments_and_equipment" />
		                <div class="overflow-auto" v-else>
		                    <table id="list_diy" role="table" aria-busy="false"
		                           :aria-colcount="fields_instruments_and_equipment.length"
		                           class="table b-table table-striped table-hover">
		                        <thead>
		                        <tr>
		                            <th v-for="(o,i) in fields_instruments_and_equipment" :key="i">
		                                {{o.label}}
		                            </th>
		                        </tr>
		                        </thead>
		                        <tbody>
		                        <tr v-for="(o, i) in list_table_instruments_and_equipment" :key="i" @click="to_details('instruments_and_equipment',o,'instruments_and_equipment_id')">
		                            <td v-for="(oj,n) in fields_instruments_and_equipment">
		                                <img v-if="oj.type && oj.type == '图片' " :src="$fullUrl(o[oj.key])" alt=""
		                                     v-default-img="'/img/default.png'">
		        ...
		        <div class="row">
		            <div class="col-12">
		                ...
		                <div class="diy_recommend">
		                    ...
		                    <div class="more_box">
		                        <router-link to="/learning_materials/list" class="more">
		                            <span>更多</span>
		                        </router-link>
		                    </div>
		                    <div class="switch" @click="show_list_learning_materials = !show_list_learning_materials">
		                        <div class="switch_box">
		                            <span v-if="show_list_learning_materials"> 切换表格 </span>
		                            <span v-else> 切换列表 </span>
		                        </div>
		                    </div>
		                </div>
		                <!--学习资料推荐列表组件 -->
		                <list_learning_materials v-if="show_list_learning_materials" :list="list_learning_materials" />
		                <div class="overflow-auto" v-else>
		                    <table id="list_diy" role="table" aria-busy="false"
		                           :aria-colcount="fields_learning_materials.length"
		                           class="table b-table table-striped table-hover">
		                        <thead>
		                        <tr>
		                            <th v-for="(o,i) in fields_learning_materials" :key="i">
		                                {{o.label}}
		                            </th>
		                        </tr>
		                        </thead>
		                        <tbody>
		                        <tr v-for="(o, i) in list_table_learning_materials" :key="i" @click="to_details('learning_materials',o,'learning_materials_id')">
		                            <td v-for="(oj,n) in fields_learning_materials">
		                                <img v-if="oj.type && oj.type == '图片' " :src="$fullUrl(o[oj.key])" alt=""
		                                     v-default-img="'/img/default.png'">
										<span v-else-if="oj.type && oj.type == 'UID' ">
											<span style="display: none">
	...
	export default {
		...
		data() {
			return {
				...
				show_list_instruments_and_equipment: true,
				list_instruments_and_equipment: [],
				vm_arr_instruments_and_equipment: [
					"instruments_and_equipment_id",
					"instrument_no",
					"instrument_name",
					"instrument_category",
					"purchase_date",
				...
				fields_instruments_and_equipment: [
					{
						key: "instrument_no",
						label: "仪器编号",
						type: "文本"
				...
				show_list_learning_materials: true,
				list_learning_materials: [],
				vm_arr_learning_materials: [
		...
		methods: {
			...
			// 获取仪器设备列表
			get_instruments_and_equipment() {
				let url = "~/api/instruments_and_equipment/get_list?";
				this.$get(url, {
					"page": 1,
					"size": 4
				}, (json) => {
					if (json.result) {
						this.list_instruments_and_equipment = json.result.list;
					}
				})
			},
			// 获取学习资料列表
			get_learning_materials() {
				let url = "~/api/learning_materials/get_list?";
				this.$get(url, {
					"page": 1,
					"size": 4
				}, (json) => {
					if (json.result) {
						this.list_learning_materials = json.result.list;
					}
				})
			},
		...
		mounted() {
			this.get_instruments_and_equipment();
			this.get_learning_materials();
			this.get_menu();
			this.get_slides();
			this.get_article();
			this.get_link();
		...
		computed: {
		    list_table_instruments_and_equipment() {
		        var list = this.list_instruments_and_equipment;
		        var list_table = [];
		        for (let i = 0; i < list.length; i++) {
		            list_table[i] = {};
		            this.vm_arr_instruments_and_equipment.map((o) => {
		                // 第二个中括号是对象的属性
		    ...
		    list_table_learning_materials() {
		        var list = this.list_learning_materials;
		        var list_table = [];
		        for (let i = 0; i < list.length; i++) {
		            list_table[i] = {};
		            this.vm_arr_learning_materials.map((o) => {
		                // 第二个中括号是对象的属性
		                list_table[i][o] = list[i][o] || "";
		            });
		        }
